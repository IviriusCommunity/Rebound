...
int WINAPI RunFileDlg_Hook(HWND hwnd, HICON icon, LPCWSTR path, LPCWSTR title, LPCWSTR prompt, UINT flags)
{
    ...
    {
		MessageBoxW(NULL, L"Rebound Shell is not currently running. The Run dialog hook is enabled, but the Rebound Shell process could not be found. Please start Rebound Shell or disable the Run dialog hook in settings.", L"Rebound Shell Not Running", MB_OK | MB_ICONWARNING);
