...
public static class IFEOEngine
{
    /*public static async Task PauseIFEOEntryAsync(string executableName)
    {
        ...
        {
            var script = $@"
if (Test-Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\{executableName}') {{
    Copy-Item -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\{executableName}' -Destination 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\INVALID{executableName}' -Recurse
    Remove-Item -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\{executableName}' -Recurse
}}
";
    ...
    public static async Task ResumeIFEOEntryAsync(string executableName)
    {
        ...
        {
            var script = $@"
if (Test-Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\INVALID{executableName}') {{
    Copy-Item -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\INVALID{executableName}' -Destination 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\{executableName}' -Recurse
    Remove-Item -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\INVALID{executableName}' -Recurse
}}
";
    ...
    public static async Task PauseIFEOEntryAsync(string executableName)
    {
        try
        {
            ...
            string originalKey = $"{basePath}\\{executableName}";
            string newKey = $"{basePath}\\INVALID{executableName}";
            ...
            await Task.CompletedTask;  // Placeholder for async method if needed
        }
        catch (Exception ex)
        {
            // Handle exceptions/logging if needed
    ...
    public static async Task ResumeIFEOEntryAsync(string executableName)
    {
        try
        {
            var basePath = @"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options";
            string originalKey = $"{basePath}\\{executableName}";
            string invalidKey = $"{basePath}\\INVALID{executableName}";
